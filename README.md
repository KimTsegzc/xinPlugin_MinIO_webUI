...
DSH（DeepSeek Harness）原生融合的 **知识库 / MinIO 文件管理插件** —— V3.2.1
...
> 📌 **V3.2.1 补丁**：修复「先前命令行/standalone 入库的文件在 UI 显示未入库」——新增宿主 `reconcileIngest`（从 Chroma 的 `list_sources` 对齐状态），工具栏新增「**刷新入库**」按钮，点一次即把已在 Chroma 的文件状态核正为已入库（含片段数）。
>
...
>
> 📌 **V2.5 多选 + 批量删除**：工具栏「多选」进入多选模式——文件卡片/列表行出现勾选角标，点选切换（文件夹仍导航）；顶部批量栏显示已选数量、全选/取消全选、删除选中（二次确认，循环调用宿主 remove，仅删除文件、不动目录）。退出多选清空选择。
>
> 📌 **V2.4 搜索/过滤**：工具栏新增搜索框，按「文件名 / 类型扩展名」实时过滤（对文件夹与文件都生效，过滤后再排序）。
>
> 📌 **V2.3 视图偏好记忆 + 文件详情**：图标/列表视图与排序字段/方向记忆在 `localStorage`，重开面板/刷新后沿用；预览弹窗新增详情条显示 类型/大小/修改时间。
>
